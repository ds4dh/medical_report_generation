"""
Medical transcript generator using vLLM
...
def create_ehr_simulation_prompt( input_ehr: str, report_type_text: str, 
                                rep_specialty_text: str) -> Tuple[str, str]:
    """
    Create a structured prompt for medical transcript generation from an input EHR.
    ...
    role_description = f'As an experienced {rep_specialty_text} physician, you are tasked with producing a detailed and structured {report_type_text} based solely on the provided medical information.'

    question = f''' ***** Objective: Create a professional {report_type_text} as a medical specialist in {rep_specialty_text} *****

        As an experienced {rep_specialty_text} physician, you are tasked with producing a detailed and structured {report_type_text} based solely on the provided medical information.

        ***** Methodology to follow: *****
        1. Carefully analyze each provided medical entity
        ...
        8. Write in the formal, professional style typical of {report_type_text} medical documentation

        ***** Instructions for your {report_type_text}: *****
        - Create a {report_type_text} that would be recognized as authentic by other {rep_specialty_text} specialists
        - Include all sections typically found in a {report_type_text} report
...
def validate_input_file(file_path: str, language: str) -> None:
    ...
    required_columns = ['MT_id',   f'{language}_report', f'{language}_EHR', f'{language}_specialty',f'{language}_report_type']
    ...
    try:
        ...
        with open(output_file_path, 'w', newline='', encoding='utf-8') as csvfile:
            fieldnames = [
                'MT_id',
                'EHR',
                f'{language}_reference_report',
                f'{language}_generated_report',  # Added comma here
            ...
            for batch_idx in range(0, total_samples, batch_size):
                ...
                all_messages = []
                for idx in range(len(EHR_batch)):
                    ...
                    try:
                        ...
                        for j, output in enumerate(batch_outputs):
                            ...
                            instance = {
                                'MT_id': mt_id_batch[j],
                                'EHR': EHR_batch[j],
                                f'{language}_reference_report':  med_tr_batch[j],
                                f'{language}_generated_report': generated_text, 
