import pandas as pd
import os
import random

random.seed(42)


def create_evaluation_excel(df_human, df_machine, save_name):
    """
    Create an Excel file with human and machine-generated reports for evaluation.
    
    Args:
        df_human: DataFrame containing human-generated reports
        df_machine: DataFrame containing machine-generated reports
        save_name: Base name for the output file
    
    Returns:
        DataFrame with shuffled reports and metadata
    """
    output_file = f'{save_name}evaluation.xlsx'
    
    # Create empty lists to store all data
    entities = []
    reports = []
    agents = []
    sources = []
    specialties = []
    report_types = []
    accuracies = []
    fluencies = []
    completeness = []
    true_texts = []
    
    # Add human reports with their metadata
    for _, row in df_human.iterrows():
        entities.append(row['input_entities'])
        reports.append(row['true_text'])
        agents.append("human")
        sources.append(row['source'])
        specialties.append(row['specialty'])
        report_types.append(row['report_type'])
        true_texts.append(row['true_text'])
        accuracies.append('')
        fluencies.append('')
        completeness.append('')
    
    # Add machine reports with their metadata
    for _, row in df_machine.iterrows():
        entities.append(row['input_entities'])
        reports.append(row['generation'])
        agents.append("machine")
        sources.append(row['source'])
        specialties.append(row['specialty'])
        report_types.append(row['report_type'])
        true_texts.append(row['true_text'])
        accuracies.append('')
        fluencies.append('')
        completeness.append('')
    
    # Create DataFrame and shuffle
    df = pd.DataFrame({
        'EHR': entities,
        'true_texts': true_texts,
        "Report": reports,
        "Agent": agents,
        "Source": sources,
        "Specialty": specialties,
        "Report type": report_types,
        'Accuracy': accuracies,
        'Fluency': fluencies,
        'Completeness': completeness,
    })
    df = df.sample(frac=1, random_state=42).reset_index(drop=True)
    
    # Save to Excel
    df.to_excel(output_file, index=False)
    print(f"Excel file created successfully: {output_file}")
    
    return df


def main(input_folder, output_folder, n_human=n_h, n_machine=n_m):
    """
    Main function to process CSV files and create evaluation Excel files.
    
    Args:
        input_folder: Path to folder containing input CSV files
        output_folder: Path to folder where output Excel files will be saved
        n_human: Number of human reports to sample (default: n_h)
        n_machine: Number of machine reports to sample (default: n_m)
    """
    # Create output directory if it doesn't exist
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)
    
    # Get sorted CSV files containing 'score' in filename
    csv_files = sorted([file for file in os.listdir(input_folder)
                       if file.endswith('.csv') and 'score' in file])
    
    for cv_file in csv_files:
        # Read CSV
        df = pd.read_csv(os.path.join(input_folder, cv_file), encoding='utf-8')
        
        # Sample human and machine reports
        df_human = df.sample(n=n_human)
        df_machine = df.sample(n=n_machine)
        
        # Determine language and save path
        lang = 'ENG' if 'ENG' in cv_file else 'FRE'
        save_add_name = (f'{output_folder}/Task2_case_reports_{lang}_'
                        if 'PubMed' in cv_file
                        else f'{output_folder}/Task2_medical_transcripts_{lang}_')
        
        create_evaluation_excel(df_human, df_machine, save_add_name)


if __name__ == "__main__":
    # Configure your paths here
    INPUT_FOLDER = 'path/to/your/input/folder'  # Replace with your input folder path
    OUTPUT_FOLDER = 'path/to/your/output/folder'  # Replace with your output folder path
    
    # Optional: customize sample sizes
    N_HUMAN_SAMPLES = n_h
    N_MACHINE_SAMPLES = n_m
    
    main(INPUT_FOLDER, OUTPUT_FOLDER, N_HUMAN_SAMPLES, N_MACHINE_SAMPLES)
